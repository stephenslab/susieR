...
test_that("susie ELBO is monotonically increasing for NIG residuals with L = 1 (data_small)", {
  ...
  fit <- suppressWarnings(
    susie_additive(data_small$X, data_small$y, L = 1,
...
test_that("susie niter is at most max_iter and converged field is present", {
  ...
  fit <- susie_additive(dat$X, dat$y, L = 5, max_iter = 100, verbose = FALSE)
...
test_that("susie respects L parameter", {
  ...
  fit_L3 <- susie_additive(dat$X, dat$y, L = 3, verbose = FALSE)
  fit_L7 <- susie_additive(dat$X, dat$y, L = 7, verbose = FALSE)
...
test_that("susie caps L at p when L > p", {
  ...
  fit <- susie_additive(dat$X, dat$y, L = 50, verbose = FALSE)
...
test_that("susie standardize=TRUE sets positive X_column_scale_factors; both variants produce valid output", {
  ...
  fit_std   <- susie_additive(dat$X, dat$y, L = 5, standardize = TRUE,  verbose = FALSE)
  fit_nostd <- susie_additive(dat$X, dat$y, L = 5, standardize = FALSE, verbose = FALSE)
...
test_that("susie intercept=TRUE gives finite intercept; intercept=FALSE gives 0", {
  ...
  fit_int   <- susie_additive(dat$X, dat$y, L = 5, intercept = TRUE,  verbose = FALSE)
  fit_noint <- susie_additive(dat$X, dat$y, L = 5, intercept = FALSE, verbose = FALSE)
...
test_that("susie prior_weights shifts posterior mass toward favored variables", {
  ...
  fit_uniform <- susie_additive(dat$X, dat$y, L = 5, verbose = FALSE)
  fit_custom  <- susie_additive(dat$X, dat$y, L = 5, prior_weights = custom_weights, verbose = FALSE)
...
test_that("susie null_weight=0 gives p alpha columns; null_weight>0 gives p+1", {
  ...
  fit_nonull <- susie_additive(dat$X, dat$y, L = 5, null_weight = 0,   verbose = FALSE)
  fit_null   <- susie_additive(dat$X, dat$y, L = 5, null_weight = 0.1, verbose = FALSE)
...
test_that("susie estimate_residual_variance=FALSE fixes sigma2 at specified value", {
  ...
  fixed_sigma2 <- 1.5
  fit_fixed <- susie_additive(dat$X, dat$y, L = 5,
                     estimate_residual_variance = FALSE,
                     residual_variance = fixed_sigma2,
                     verbose = FALSE)
  fit_est <- susie_additive(dat$X, dat$y, L = 5,
...
test_that("susie convergence_method pip and elbo both produce valid output", {
  ...
  fit_elbo <- susie_additive(dat$X, dat$y, L = 5, convergence_method = "elbo", verbose = FALSE)
  fit_pip  <- susie_additive(dat$X, dat$y, L = 5, convergence_method = "pip",  verbose = FALSE)
...
test_that("susie compute_univariate_zscore=TRUE attaches z of length p; FALSE gives NULL", {
  ...
  fit_noz <- susie_additive(dat$X, dat$y, L = 5, compute_univariate_zscore = FALSE, verbose = FALSE)
  fit_z   <- susie_additive(dat$X, dat$y, L = 5, compute_univariate_zscore = TRUE,  verbose = FALSE)
...
test_that("susie estimate_residual_method MoM produces positive sigma2", {
  ...
  fit <- susie_additive(dat$X, dat$y, L = 5,
...
test_that("susie estimate_residual_method NIG (L=1) produces positive sigma2 and monotone ELBO", {
  ...
  fit <- susie_additive(dat$X, dat$y, L = 1,
               estimate_residual_method = "NIG",
               verbose = FALSE)
...
test_that("susie NIG rejects improper Inverse-Gamma priors (alpha0=0 or beta0=0)", {
  ...
  expect_error(
    susie_additive(dat$X, dat$y, L = 1,
  ...
  expect_error(
    susie_additive(dat$X, dat$y, L = 2,
          min_abs_corr = 0, check_null_threshold = -1000,
          estimate_residual_method = "NIG",
          alpha0 = 0, beta0 = 0.5,
          verbose = FALSE),
  ...
  fit_l1 <- susie_additive(dat$X, dat$y, L = 1,
                  min_abs_corr = 0, check_null_threshold = -1000,
                  estimate_residual_method = "NIG",
  ...
  fit_l2 <- susie_additive(dat$X, dat$y, L = 2,
                  min_abs_corr = 0, check_null_threshold = -1000,
                  estimate_residual_method = "NIG",
                  alpha0 = 0.1, beta0 = 0.1,
                  verbose = FALSE)
...
test_that("susie estimate_prior_method options all produce valid ELBO-monotone fits", {
  ...
  for (method in c("optim", "EM", "simple")) {
    fit <- susie_additive(dat$X, dat$y, L = 5, estimate_prior_method = method, verbose = FALSE)
...
test_that("susie unmappable_effects=none does not add theta/tau2 fields", {
  ...
  fit <- susie_additive(dat$X, dat$y, L = 5, unmappable_effects = "none", verbose = FALSE)
...
test_that("susie unmappable_effects=inf adds theta/tau2 of correct length", {
  ...
  fit <- susie_additive(dat$X, dat$y, L = 5, unmappable_effects = "inf", verbose = FALSE)
...
test_that("susie drops unmappable fields when SuSiE-inf model_init used with none", {
  ...
  fit_inf <- susie_additive(dat$X, dat$y, L = 5, unmappable_effects = "inf",
                   refine = FALSE, verbose = FALSE)
  fit <- susie_additive(dat$X, dat$y, L = 5, unmappable_effects = "none",
...
test_that("susie unmappable_effects=ash adds theta/tau2", {
  ...
  fit <- susie_additive(dat$X, dat$y, L = 5, unmappable_effects = "ash", verbose = FALSE)
...
test_that("susie handles small p (p=5)", {
  ...
  fit <- susie_additive(dat$X, dat$y, L = 3, verbose = FALSE)
...
test_that("susie near-zero signal y produces valid output with small PIPs", {
  ...
  fit <- suppressWarnings(susie_additive(X, y, L = 5, verbose = FALSE))
...
test_that("susie errors on NA values when na.rm = FALSE", {
  ...
  expect_error(
    susie_additive(dat$X, dat$y, L = 5, na.rm = FALSE, verbose = FALSE),
...
test_that("susie_ss PIPs, V, and sigma2 agree with susie on the same data", {
  ...
  fit_ind <- susie_additive(dat$X, dat$y, L = 5, standardize = TRUE, verbose = FALSE)
...
test_that("susie, susie_ss, and susie_rss give similar PIPs on the same data", {
  ...
  fit_ind <- susie_additive(dat$X, dat$y, L = 5, standardize = TRUE,
...
test_that("at least one of the three interfaces finds credible sets on strong signal data", {
  ...
  fit_ind <- susie_additive(dat$X, dat$y, L = 10, verbose = FALSE)
...
test_that("susie refine=TRUE produces ELBO >= refine=FALSE (up to tolerance)", {
  ...
  fit_norefine <- susie_additive(dat$X, dat$y, L = 10, refine = FALSE, verbose = FALSE)
  fit_refine   <- susie_additive(dat$X, dat$y, L = 10, refine = TRUE,  verbose = FALSE)
...
test_that("susie track_fit=TRUE attaches a susie_track trace object", {
  ...
  fit <- susie_additive(dat$X, dat$y, L = 5, track_fit = TRUE, verbose = FALSE)
...
test_that("susie verbose=TRUE emits ELBO messages", {
  ...
  expect_message(
    susie_additive(dat$X, dat$y, L = 5, verbose = TRUE),
    "ELBO"
  )
})
...
test_that("all interfaces maintain non-negative prior variances", {
  ...
  fit_ind <- susie_additive(dat$X, dat$y, L = 5, verbose = FALSE)
