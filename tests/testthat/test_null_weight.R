...
test_that("SS: null weight specification agrees with default",{
  simulate(200,1000)
  ss = compute_ss(X,y)

  res1 = susie_ss(cbind(rbind(ss$XtX,0),0), c(ss$Xty, 0), n = ss$n, var_y = var(y),
                  estimate_prior_variance = TRUE, estimate_residual_variance = TRUE)

  res2 = susie_ss(ss$XtX, ss$Xty, n = ss$n, var_y = var(y),
                  estimate_prior_variance = TRUE, estimate_residual_variance = TRUE,
