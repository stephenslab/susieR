...
test_that("susie for trend filtering",{
  ...
  with(simulate_tf(0), {
    ...
    original.s0.ns.ni = susie(X,y,standardize=FALSE,intercept=FALSE)
    s0.ns.ni = susie_trendfilter(y,0,standardize=FALSE,intercept=FALSE)
    original.s0.ns = susie(X,y,standardize=FALSE,intercept=TRUE)
    s0.ns = susie_trendfilter(y,0,standardize=FALSE,intercept=TRUE)
    original.s0.ni = susie(X,y,standardize=TRUE,intercept=FALSE)
    s0.ni = susie_trendfilter(y,0,standardize=TRUE,intercept=FALSE)
...
test_that("susie_trendfilter interface",{
  with(simulate_tf(0), {
    ...
    expect_error(susie_trendfilter(y,0,standardize=FALSE,intercept=FALSE), NA)
    expect_error(susie_trendfilter(y,0,standardize=FALSE,intercept=TRUE), NA)
    expect_error(susie_trendfilter(y,0,standardize=TRUE,intercept=FALSE), NA)
