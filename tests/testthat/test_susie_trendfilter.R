...
test_that("susie for trend filtering",{
  ...
  with(simulate_tf(0), {
    ...
    s0 = susie_trendfilter(y,0,estimate_prior_variance = FALSE)
    original.s0.ns.ni = susie(X,y,standardize=FALSE,intercept=FALSE,estimate_prior_variance = FALSE)
    s0.ns.ni = susie_trendfilter(y,0,standardize=FALSE,intercept=FALSE,estimate_prior_variance = FALSE)
    original.s0.ns = susie(X,y,standardize=FALSE,intercept=TRUE,estimate_prior_variance = FALSE)
    s0.ns = susie_trendfilter(y,0,standardize=FALSE,intercept=TRUE,estimate_prior_variance = FALSE)
    original.s0.ni = susie(X,y,standardize=TRUE,intercept=FALSE,estimate_prior_variance = FALSE)
    s0.ni = susie_trendfilter(y,0,standardize=TRUE,intercept=FALSE,estimate_prior_variance = FALSE)
  ...
  with(simulate_tf(1), {
    suppressWarnings({
      ...
      s1 <- susie_trendfilter(y,1,estimate_prior_variance = FALSE)
      original.s1.ns.ni <- susie(X,y,standardize=FALSE,intercept=FALSE,estimate_prior_variance = FALSE)
      s1.ns.ni <- susie_trendfilter(y,1,standardize=FALSE,intercept=FALSE,estimate_prior_variance = FALSE)
      original.s1.ns <- susie(X,y,standardize=FALSE,intercept=TRUE,estimate_prior_variance = FALSE)
      s1.ns <- susie_trendfilter(y,1,standardize=FALSE,intercept=TRUE,estimate_prior_variance = FALSE)
      original.s1.ni <- susie(X,y,standardize=TRUE,intercept=FALSE,estimate_prior_variance = FALSE)
      s1.ni <- susie_trendfilter(y,1,standardize=TRUE,intercept=FALSE,estimate_prior_variance = FALSE)
    })
  ...
  with(simulate_tf(2), {
    ...
    s2 = susie_trendfilter(y,2,estimate_prior_variance = FALSE)
    original.s2.ns.ni = susie(X,y,standardize=FALSE,intercept=FALSE,estimate_prior_variance = FALSE)
    s2.ns.ni = susie_trendfilter(y,2,standardize=FALSE,intercept=FALSE,estimate_prior_variance = FALSE)
    original.s2.ns = susie(X,y,standardize=FALSE,intercept=TRUE,estimate_prior_variance = FALSE)
    s2.ns = susie_trendfilter(y,2,standardize=FALSE,intercept=TRUE,estimate_prior_variance = FALSE)
    original.s2.ni = susie(X,y,standardize=TRUE,intercept=FALSE,estimate_prior_variance = FALSE)
    s2.ni = susie_trendfilter(y,2,standardize=TRUE,intercept=FALSE,estimate_prior_variance = FALSE)
...
test_that("susie_trendfilter interface",{
  with(simulate_tf(0), {
    ...
    expect_error(susie_trendfilter(y,0,null_weight=1/(n+1),estimate_prior_variance = FALSE), NA)
    expect_error(susie_trendfilter(y,0,estimate_prior_variance=TRUE, null_weight=1/(n+1)), NA)
    expect_error(susie_trendfilter(y,0,standardize=FALSE,intercept=FALSE,estimate_prior_variance = FALSE), NA)
    expect_error(susie_trendfilter(y,0,standardize=FALSE,intercept=TRUE,estimate_prior_variance = FALSE), NA)
    expect_error(susie_trendfilter(y,0,standardize=TRUE,intercept=FALSE,estimate_prior_variance = FALSE), NA)
    expect_error(susie_trendfilter(y,0,compute_univariate_zscore =TRUE,estimate_prior_variance = FALSE), NA)
  })
})
