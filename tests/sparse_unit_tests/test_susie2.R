...
test_that("sparse version susie",{
  original.res = readRDS('../original_susie_results/susiefit_original_res2.rds')
  set.seed(1)
  n = 1000
  p = 10000
  beta = rep(0,p)
  ...
  dense.res = susie(X.dense, y, standardize=TRUE, intercept = FALSE)
  sparse.res = susie(X.sparse, y, standardize=TRUE, intercept = FALSE)
