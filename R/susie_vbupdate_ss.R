...
update_each_effect_ss <- function (XtX, Xty, s_init, estimate_prior_variance=FALSE) {
  ...
  s = s_init
  L = nrow(s$alpha)
  dXtX = Matrix::diag(XtX)

  if(L>0){
    for (l in 1:L){
      # remove lth effect from fitted values
      s$XtXr = s$XtXr - as(XtX %*% (s$alpha[l,] * s$mu[l,]), 'matrix')

      #compute residuals
      XtR = Xty - s$XtXr
      res = single_effect_regression_ss(XtR,dXtX,s$V[l],s$sigma2,estimate_prior_variance)
