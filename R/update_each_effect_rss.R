...
#' @keywords internal
update_each_effect_rss <- function (R, z, s, estimate_prior_variance=FALSE,estimate_prior_method="optim") {
  ...
  if(L>0){
    for (l in 1:L){
      ...
      #compute residuals
      r = z - s$Rz
      res = single_effect_regression_rss(as.vector(r),R,s$V[l], s$sigma2, s$pi,estimate_prior_method)
      ...
      s$KL[l] <- -res$lbf_model + SER_posterior_e_loglik_rss(R,r, s$sigma2, res$alpha*res$mu,res$alpha*res$mu2)
