...
#'
#' @param t an n vector of locations of observations in x axis (time/space). Default is uniformly spaced on [0,1].
#'
#' @param breaks an n vector of breaks where changepoints may occur (default is observation locations)
#'
...
#' @export
susie_tfg = function(y, t=seq(0,1,length=length(y)), breaks=sort(t),  order=0,standardize=FALSE, use_mad=FALSE,...){
  warning("this function is in development and not yet intended  for practical use")
  if (order > 0){
    warning("order>0 is not recommended (see ?susie_trendfilter for more explanation).")
  }
  n = length(y)

  X <- susieR:::make_tfg_matrix(t,breaks,order)
  if (use_mad){
