...
#'         attr(R, 'eigen') is the eigen decomposition of R.
set_R_attributes = function(R, r_tol){
  ...
  eigenR$vectors = eigenR$vectors[, eigenR$values!=0]
  eigenR$values = eigenR$values[eigenR$values!=0]
  res = eigenR$vectors %*% (t(eigenR$vectors) * eigenR$values)

  attr(res, 'eigen') = eigenR
  attr(res, 'd') <- diag(res)
  attr(res, 'scaled:scale') <- rep(1, length = nrow(R))
  attr(res, 'RinvRj') = tcrossprod(eigenR$vectors)
