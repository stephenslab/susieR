...
The data-set additionally provides summary statistics for fitting univariate simple regression variable by variable. The results are $\hat{\beta}$ and $SE(\hat{\beta})$ from which $\hat{z}$-scores can be derived. Again we focus only on results from the first data-set:

```{r}
z_hat <- with(finemap,sumstats[1,,] / sumstats[2,,])
z_hat <- z_hat[,1]
susie_plot(z_hat, y = "z", b=b)
```
...
## Fine-mapping with `susieR` using bhat, shat, R and n

For starters, we assume there are at most 5 causal variables, i.e. set `L=5`, although SuSiE is robust to the choice of `L`.

We set SuSiE prior variance to 0.1. The `susieR` function call is:

```{r}
fitted_bhat <- susie_bhat(bhat = finemap$sumstats[1,,1], shat = finemap$sumstats[2,,1], 
                          R = R, R_type = 'Cor', n = nrow(finemap$data$X), 
                          L = 5, 
                          scaled_prior_variance = 0.1, 
                          estimate_prior_variance = TRUE, verbose = TRUE)
```

Using `summary` function, we can see the posterior inclusion probability (PIP) for each variable, and the confidence sets.
...
## Fine-mapping with `susieR` using z scores, R and n

We generate the z scores from the p values
```{r}
...
We assume there are at most 5 causal variables, i.e. set `L=5`.

We set SuSiE prior variance to 0.1. The `susieR` function call is:

```{r}
fitted_z <- susie_z(z = z_scores,R = R, R_type='Cor', n = nrow(finemap$data$X),
                    L = 5, 
                    scaled_prior_variance = 0.1, 
                    estimate_prior_variance = TRUE, verbose = TRUE)
```
