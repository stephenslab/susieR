...
(LD) matrix, ${\bf R}$ (without standardizing), which will be used to
run `susie_rss`.
...
The `susie` and `susie_rss` analyses produce the exact same results
when the summary statistics `bhat`, `shat` and `var_y` (and `n`) are
provided to `susie_rss`.
...
divided by the standard deviation so that they all have the same
standard deviation.
...
`bhat`, `shat`), `R` and `n` should allow `susie_rss` to recover the
same results as `susie`:
...
When possible we recommend estimating the residual variance.
...
```{r fourth-comparison, fig.height=3.5, fig.width=3, message=FALSE}
