...
* **Median / OR purity filtering**: New `median_abs_corr` purity threshold applied to the median absolute correlation among credible-set variables, OR-linked with the existing `min_abs_corr` (minimum) threshold so a set is kept if it clears either criterion; `min_abs_corr` can now be set to `NULL` to filter on the median alone.
...
* **SuSiE-inf speedup**: Low-rank matrix factorizations replace the full eigendecomposition in both SuSiE-inf backends (individual-level and summary statistics), accelerating the n ≪ p (e.g. QTL) regime.
