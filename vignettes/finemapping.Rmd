...
For starters, we assume there are at most 5 causal variables, i.e.,
set `L=5`, although SuSiE is robust to the choice of `L`.
...
By default we output 95% confidence set:
...
If we use the default 90% coverage for confidence sets, we still
...
```{r}
z3 = cbind(fitted$sets$cs[[3]], z_scores[fitted$sets$cs[[3]]],
           fitted$pip[sets$cs[[3]]])
...
[N3finemapping]: https://github.com/stephenslab/susieR/blob/master/inst/data/N3finemapping.rds
