...
The simulated data-set is [available here](https://github.com/stephenslab/susieR/blob/master/inst/data/N3finemapping.rds), as part of the `susieR` package. It is a simulate to have exactly 3 non-zero effects.
...
```{r}
z3 = cbind(sets$cs[[3]], z_scores[sets$cs[[3]]])
...
Using an average $r^2$ filter of 0.2 ($r=0.44$ compared to SuSiE's $r=0.1$ in this analysis), it reports 5 95% CS. But only the first 3 CS contain causal signals. 
