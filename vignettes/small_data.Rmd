...
This is vignette mimic the example from Figure 1 panel B-C-D in [Denault et al paper](https://doi.org/10.1101/2025.05.16.654543).
...
## Data
We analyze a simulated eQTL dataset built from normalized gene expression and normalized SNP genotypes. The goal is to fine-map causal variants for expression (eQTLs). In this example the ground truth is unknown.
...
Setting `small = TRUE` switches SuSiE to a variant of single-effect regression (SER) that properly accounts for uncertainty in the residual , see following Servin & Stephens (2007, PLOS Genetics).

Setting `small = TRUE` in the susie call uses a slightly different  single effect regression
(i.e. Servin-Stephens single effect regression, see Servin and Stephens  2007 PLOS Genetics ) model that accounts for uncertainty in the
residual.
