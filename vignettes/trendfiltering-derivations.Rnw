...
\hat{b}_n - \hat{b}_{n-1})$ \cite{Tibshirani2014}. And when $k\geq 1$,
...
\subsection{Computation on $X^T y$} 
...
\subsection{Computation on $(X^2)^T 1$ (i.e. colSums($X^2$))} 
...
\subsection{Computation on $\mu$ (i.e. column means)} 
...
\subsection{Computation on $\sigma$ (i.e. column standard deviations)} 
...
\subsection{Computation on $(\hat{X}^2)^T 1$ (i.e. colSums($\hat{X}^2$))} 
...
\ref{Computation-on-std_d} explain how we can benefit from the unique
...
\bibliography{trendfiltering-derivations}
