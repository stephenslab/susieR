...
```{r}
fitted_z_n <- susie_z(z_scores, R = R, n = nrow(data$X), 
                      L = 5, 
                      scaled_prior_variance = 0.1, 
                      estimate_residual_variance = TRUE)
```
...
and the PIP plot
```{r}
susie_plot(fitted_z_n, y="PIP", b=b)
```

## Fine-mapping with `susieR` without the knowledge of sample size

If the sample size is not available, the user should guess the sample size. If you want conservative inference on effects, use a small sample size; if you want aggressive result, use a large sample size is large.

Here we use sample size 50 as an example
```{r}
fitted_z <- susie_z(z = z_scores,R = R, n = 50, L=1, 
                    scaled_prior_variance = 0.1, 
                    estimate_residual_variance = TRUE)
```
...
There is one confidence set that captures the true effect 773. We can check the PIP
