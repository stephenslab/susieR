...
If the sample size is not available, the user should guess the sample size. If you want conervative inference on effects, use a small sample size; if you want aggressive result, use a large sample size is large.

Here we use sample size 40 as an example
```{r}
fitted_z <- susie_z(z = z_scores,R = R, n = 40, L = 5, scaled_prior_variance = 0.1)
```

There is no confidence set. We can check the PIP
