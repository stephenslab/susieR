...
We randomly generated reference panel genotype matrix `X_ref` which contains 500 indiviudals with similar LD pattern as original genotype matrix. 
```{r echo=F}
set.seed(1234)
...
Using susie_rss function with LD from reference panel, the model identifies 10 CSes and only one of them contains the true signal. 
```{r}
