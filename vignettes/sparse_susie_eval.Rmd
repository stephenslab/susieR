...
In this minimal example, we have shown that the revised version of SuSiE can reduce computation time by around $48.6\%$, even if the input matrix is a dense matrix, and further reduce computation time $20\%$ more given the input matrix is a sparse matrix. It is also obvious that the revised version takes less memory. We provide line profiles below so that people who are interested could improve the work as well.  
