...
The slider fits partial inheritance effects using one single-effect
...
The callable package is `susieSlide::susie`. The corrected repository root
is package **susieSlide 0.2.0**, with its own IBSS engine and native library.
The previous nested-package layout was an implementation mistake and has
been removed. `susie_additive()` retains the original additive entry point.
See `ROOT_MIGRATION.md` for the layout audit and new validation logs.
...
The initial slider algorithm validation completed `R CMD check --no-manual` with **Status: OK**: no errors, warnings,
or notes. The test suite passed **379 assertions**, with no failures, warnings,
or skips. Those original logs are retained; ROOT_MIGRATION.md reports checks of the corrected root package.
