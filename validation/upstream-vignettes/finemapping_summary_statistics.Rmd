...
consistency of the LD matrix with the summary statistics. For
model-based ways to account for finite or mismatched LD reference
panels, see [this vignette](rss_mismatch.html).
...
```{r}
fitted_standardize = susie_additive(scale(X), scale(Y[,1]), L = 10)
all.equal(coef(fitted_standardize)[-1], coef(fitted_rss2)[-1])
```

### Single-effect summary-statistics analysis without LD

If the goal is to fit a one-effect credible-set model, the LD matrix is
not needed. The `susie_ser` function works directly with z-scores, or
with $\hat{\beta}$ and $SE(\hat{\beta})$, and avoids constructing an
...
Because no LD reference is supplied, this credible set does not have a
purity filter. It is the one-effect credible-set model of Maller et
al. (2012), with the attainable-coverage filter from SparsePro (Zhang
et al. 2023). To apply a purity filter afterward, call `susie_get_cs`
with an LD matrix:

```{r, eval=FALSE}
susie_get_cs(fitted_ser, Xcorr = R, coverage = 0.95)
```

To skip credible-set construction and silence the no-LD hint, set
`coverage = NULL`:

```{r}
fitted_ser_no_cs <- susie_ser(z = z_scores, n = n, coverage = NULL)
is.null(fitted_ser_no_cs$sets)
```
