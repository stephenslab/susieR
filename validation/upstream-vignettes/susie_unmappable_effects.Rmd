...
```{r}
t0 <- proc.time()
fit_susie <- susie_additive(X, y, L = 10)
t1 <- proc.time()
t1 - t0
...
```{r}
t0 <- proc.time()
fit_inf <- susie_additive(X, y, L = 10, unmappable_effects = "inf")
t1 <- proc.time()
t1 - t0
...
```{r}
t0 <- proc.time()
fit_ash <- susie_additive(X, y, L = 10, unmappable_effects = "ash", verbose = TRUE)
t1 <- proc.time()
t1 - t0
...
```{r}
t0 <- proc.time()
fit_susie_L40 <- susie_additive(X, y, L = 40)
t1 <- proc.time()
t1 - t0
